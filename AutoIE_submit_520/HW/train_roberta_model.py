...
from config import Reader, Config, evaluate_batch_insts
from config.utils_for_roberta import batching_list_instances
import time
import torch
...
def parse_arguments_t(parser):
    ...
    parser.add_argument('--train_or_predict', type=int, default=1, help="1 means train, 2 means using valid data, 3 means predict for test data")
...
def main_predict():
    ...
    with open('../submission/Hermers.json', 'w+', encoding='utf-8') as file:
        for idx in range(len(tests)):
            predict_dict = dict()
...
def main_train_valid():
    ...
    valid_model_folder = model_folder + '_valid-' + str(opt.seed)[-1]
